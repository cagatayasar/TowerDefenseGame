...
namespace Pixelplacement.TweenSystem
{
    class SplinePercentage : TweenBase
    {
        ...
        //Constructor:
        public SplinePercentage (Spline spline, Transform target, float startPercentage, float endPercentage, bool faceDirection, float duration, float delay, bool obeyTimescale, AnimationCurve curve, Func<float, float> easeFunc, Tween.LoopType loop, Action startCallback, Action completeCallback)
        {
            //clamps:
            if (!spline.loop)
            {
            ...
            SetEssentials (Tween.TweenType.Spline, target.GetInstanceID (), duration, delay, obeyTimescale, curve, easeFunc, loop, startCallback, completeCallback);
